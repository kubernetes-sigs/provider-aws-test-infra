...
if [[ "${KUBEADM_CONTROL_PLANE}" == true ]]; then
  ...
  if [[ "{{ENABLE_DRA_NVIDIA}}" == "true" ]]; then
    ...
    helm template nvidia-dra-driver nvidia/nvidia-dra-driver-gpu --version 25.8.1 --namespace kube-system --set resources.computeDomains.enabled=false --set resources.gpus.enabled=true --set nvidiaDriverRoot=/ | kubectl $KC apply -f -
